...
export async function activate(context: any) {
    ...
    context.subscriptions.push(
        ...
        vscode.workspace.onDidCloseTextDocument(async (document) => {
            if (document && hasContentFor(document.fileName)) {
                await resetAll(document.fileName);
            }
        }),
        ...
        vscode.workspace.onDidSaveTextDocument(async (document) => {
            if (!isMiniDiffEnabled(document)) return;
...
function isMiniDiffEnabled(document: vscode.TextDocument): boolean {
    ...
    // Git-based toggle
    if (config.disableInGit && utils.isFileInGitRepo(document.fileName)) {
        return false;
    }
